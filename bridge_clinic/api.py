...
def create_payment_request_for_expense(doc, method):
    ...
    try:
        ...
        pr.party_name = doc.employee_name
...
def notify_expense_payment_made(doc, method):
    ...
    try:
        ...
        frappe.msgprint("Expense Claim Payment Notification Failed")
...
def create_payment_request_from_po(doc, method):
    ...
    try:
        ...
        pr.party_name = doc.supplier_name
        pr.transaction_date = nowdate()
    ...
    try:
        ...
        for po_name in set(po_names):
            ...
            if pi.outstanding_amount and pi.outstanding_amount > 0:
                ...
                pr.party_name = pi.supplier_name
                pr.transaction_date = nowdate()
...
def set_limit_exceeded_flag(doc, method=None):
    ...
    for row in doc.expenses:
        amount = float(row.custom_rate or 0)
    ...
    # Update select field with Yes/No
    doc.custom_limit_exceeded = "Yes" if limit_exceeded else "No"
...
def update_mr_workflow_state_from_payment_request(doc, method=None):
    ...
    if doc.reference_doctype == "Purchase Invoice" and doc.docstatus == 1:
        pi = frappe.get_doc("Purchase Invoice", doc.reference_name)
        for d in pi.items:
            if d.material_request:
                frappe.db.set_value("Material Request", d.material_request, "workflow_state", "Purchase Request Approved")
